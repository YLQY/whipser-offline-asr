...
public class RunWhisper : MonoBehaviour
{
    ...
    public ModelAsset encoder_model;

    public ModelAsset decoder_model;
    public ModelAsset mel_model;
    void Start()
    {
        ...
        Model decoder = ModelLoader.Load(decoder_model);
        Model encoder = ModelLoader.Load(encoder_model);
        Model spectro = ModelLoader.Load(mel_model);

        decoderEngine = WorkerFactory.CreateWorker(backend, decoder);
        encoderEngine = WorkerFactory.CreateWorker(backend, encoder);
        spectroEngine = WorkerFactory.CreateWorker(BackendType.CPU, spectro);

        outputTokens[0] = START_OF_TRANSCRIPT;
        //outputTokens[1] = ENGLISH;// GERMAN;//FRENCH;//
        outputTokens[1] = CHINESE;
    ...
    void GetTokens()
    {
        var jsonText = FileUtils.GetTextFromStreamAssets("vocab.json");
    ...
    private string whisper_res_ori = "";
    ...
    void Update()
    {
        if (transcribe && currentToken < outputTokens.Length - 1)
        {
            ...
            {
                {"encoder_hidden_states",encodedAudio },
                {"input_ids" , tokensSoFar }
            };
            ...
            {
                whisper_res_ori += tokens[ID];
                outputString += GetUnicodeText(tokens[ID]);
            }


            textMeshPro.text = GetUnicodeText(whisper_res_ori);
        }
    }
    ...
    bool IsWhiteSpace(char c) { return !(('!' <= c && c <= '~') || ('?' <= c && c <= '?') || ('?' <= c && c <= '?')); }
